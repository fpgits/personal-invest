...
La app vive en la carpeta `invest/` del monorepo. Al conectar el repo en
Vercel, configura **Root Directory = `invest`** en Settings, General: sin eso
el build falla porque en la raiz del repo no hay package.json. Vercel lee el
`vercel.json` de esta carpeta (crons y region incluidos).

Los comandos de este README se corren desde `invest/`, no desde la raiz.
