...
/**
 * Resolver memoizado para bucles de import y sync: cada simbolo se resuelve
 * una sola vez por ejecucion aunque aparezca en 200 operaciones. Cachea la
 * promesa, asi que tambien deduplica llamadas concurrentes.
 */
export function makeAssetResolver() {
  const cache = new Map<string, Promise<Asset>>();
  return (symbol: string, assetClass: AssetClass): Promise<Asset> => {
    const key = `${assetClass}:${symbol.toUpperCase()}`;
    let hit = cache.get(key);
    if (!hit) {
      hit = ensureAsset({ symbol, assetClass });
      cache.set(key, hit);
    }
    return hit;
  };
