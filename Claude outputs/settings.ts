...
export const SETTING_KEYS = {
  ...
  // PoWERo: capital nocional de cada libro. Lo pones tu, del tamano que sea.
  poweroEquityCapital: "powero_equity_capital",
  poweroCryptoCapital: "powero_crypto_capital",
  /** manual = propone y decides; auto = apunta solo EN EL LIBRO NOCIONAL. */
  poweroMode: "powero_mode",
} as const;
...
export type PoweroSettings = {
   ...
   * el libro de mentira sin preguntarte, para poder medir el motor sin que
   * dependa de que tu estes delante. Comprar de verdad lo haces tu.
   */
  mode: "manual" | "auto";
};
...
/** Ajustes de PoWERo. Puro sobre el mapa de ajustes. */
export function poweroFromSettings(all: Record<string, string>): PoweroSettings {
  ...
  return {
    equityCapital: num(SETTING_KEYS.poweroEquityCapital, POWERO_DEFAULTS.equityCapital),
    cryptoCapital: num(SETTING_KEYS.poweroCryptoCapital, POWERO_DEFAULTS.cryptoCapital),
    mode: all[SETTING_KEYS.poweroMode] === "auto" ? "auto" : "manual",
  };
}
